...
if(!isset($_GET['pages'])){
		...
		$inboxItemPerPages = mysql_query("SELECT * FROM inbox");
		
		$totCont = mysql_num_rows($inboxItemPerPages);
		if($totCont >= 15){
			$perPages = 15;
				...
				<div class="col s12 collapsible-body" style="margin-top:30px;">
					<form  id="filterInbox" action="index.php?menu=cekinbox&pages=1&filter=on&lastID=<?php echo $lastIdMsg; ?>" method="POST">
						...
						<div class="col s3">
							...
							<input name="sender" placeholder="Name/Phone number.. (Leave blank for any number..)" id ="sender" type="text" class="validate">							
						</div>
						<div class="col s3">
							<label class="active" for="caseFilter">Cust Case</label>
							<input name="caseFilter"  id ="caseFilter" type="text" class="validate">							
						</div>
						<div class="col s12">
